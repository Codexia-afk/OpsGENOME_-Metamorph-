...
def get_demo_cross_stack_targets(
    ...
    directly from disk so live changes are immediately picked up.
    """
    ...
    for spec in DEMO_TARGET_SPECS:
        ...
        if read_from_disk_if_available and disk_path.is_file():
            try:
                ...
                targets.append((chosen_path, code, spec.error_context))
                continue
            except Exception:
                pass
...
def get_demo_cluster_issues() -> list[ClusterIssue]:
    """Returns standard SRE cluster audit matrix covering 5 simultaneous failure modes."""
    return [
        ClusterIssue(
            severity="CRITICAL",
            resource_type="Pod",
